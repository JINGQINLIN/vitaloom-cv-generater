...
def current_year_context():
    ...
    return "\n".join(
        [
            f"当前日期语境：今天是 {today.year} 年 {today.month} 月 {today.day} 日。",
            "日期判断规则：",
            "- 结束时间写「至今」「现在」「present」「current」「Now」或留空，表示仍在进行，不是未来时间，也不是错误。",
            f"- 起始年为 {today.year} 且结束为「至今」或等效表述，表示当前在职/在读/实习，完全合理。",
...
def call_ai(task, body):
    ...
    payload = {
        ...
        "messages": [
            {
                "role": "system",
                "content": "你为 VitaLoom 简历编辑器输出严格 JSON，不要 Markdown，不要 JSON 外的说明。只依据输入内容判断；不编造事实。评分与优化时：缺失姓名/联系方式绝不扣分或列为风险；「至今」「present」且起始年≤当前年的时间线完全合法，不得要求澄清。解释性文字使用请求中的 outputLang；proposal 必须与原字段同语言；不要把切换语言当作优化建议。",
            },
